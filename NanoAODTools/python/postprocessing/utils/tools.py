...
def get_jet(jets):
    goodjet = list(filter(lambda x : x.jetId and x.pt>25 and abs(x.eta)<2.5 , jets))
    return goodjet

def get_fatjet(fatjets):
    goodfatjets = list(filter(lambda x : x.jetId and abs(x.eta) < 2.5 and x.pt>200, fatjets))
    return goodfatjets
